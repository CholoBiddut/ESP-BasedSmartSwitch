...
String themeCss() {
  if (cfg.theme == 1) return "body{margin:0;font-family:Arial,sans-serif;background:#0f172a;color:#e2e8f0}.card{background:#111827}";
  if (cfg.theme == 2) return "body{margin:0;font-family:Arial,sans-serif;background:#f8fafc;color:#0f172a}.card{background:#ffffff}";
  return "body{margin:0;font-family:Arial,sans-serif;background:#f4f7fb;color:#111}.card{background:#fff}";
}
...
String htmlPage() {
  ...
  s += ".card{border-radius:18px;padding:14px;box-shadow:0 8px 22px rgba(0,0,0,.08);margin-bottom:12px}";
  ...
  s += ".pwbtn{margin-top:8px;padding:14px 12px;border:0;border-radius:14px;background:#e2e8f0;font-size:14px;font-weight:700}";
  ...
  s += "</div><div class='footer'>Powered by CholoBiddut © 2026 CholoBiddut.</div>";

  s += "<script>";
  s += "let ws;function connect(){ws=new WebSocket('ws://'+location.host+':81/');ws.onopen=()=>ws.send('get');";
  s += "ws.onmessage=e=>{let d=JSON.parse(e.data);ssid.textContent=d.ssid;ip.textContent=d.ip;apSsid.value=d.ssid;sw1Name.value=d.n1;sw2Name.value=d.n2;mdnsName.value=d.mdns;timerSec.value=d.d;theme.value=d.theme;";
  s += "upd(1,d.r1,d.n1);upd(2,d.r2,d.n2);c1.textContent=d.t1;c2.textContent=d.t2;};";
