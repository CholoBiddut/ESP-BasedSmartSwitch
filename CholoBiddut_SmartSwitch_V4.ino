...
String pageHtml() {
  ...
  s += "<div id='setupInfo' class='info hidden'><div class='small'>Press and hold the configure button for 5 seconds to enter AP mode.</div></div></div>";
  ...
  s += "function toggleSetup(){document.getElementById('setupInfo').classList.toggle('hidden');}";
  s += "</script></body></html>";
  return s;
}
...
void startPortal() {
 ...
 std::vector<const char *> menuItems = {"wifi", "update"};
  wm.setMenu(menuItems);

  bool ok = wm.startConfigPortal(apName);
  if (ok && WiFi.status() == WL_CONNECTED) portalRunning = false;
}
...
void setupWiFi() {
  ...
  if (!ok || WiFi.status() != WL_CONNECTED) portalRunning = true;
}
