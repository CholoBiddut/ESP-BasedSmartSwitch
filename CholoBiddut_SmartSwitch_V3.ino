...
void sendStateToClients() {
  String wifiName = "Disconnected";
  if (WiFi.status() == WL_CONNECTED) wifiName = WiFi.SSID();
  else if (portalRunning) wifiName = apName;

  String payload = "{";
  payload += "\"r1\":" + String(relay1State ? "true" : "false") + ",";
  payload += "\"r2\":" + String(relay2State ? "true" : "false") + ",";
  payload += "\"wifi\":\"" + escapeJson(wifiName) + "\",";
...
String pageHtml() {
  ...
  s += ".wrap{max-width:520px;margin:0 auto;padding:14px 14px 84px}";
  ...
  s += ".info{margin-top:10px;padding:12px;border-radius:14px;background:#f8fafc;border:1px solid #e2e8f0}";
  ...
  s += "<div id='setupInfo' class='info hidden'><div class='small'>Press and hold the configure button for 5 seconds to enter AP mode.</div><div class='small' style='margin-top:8px'>When AP mode starts, connect your phone to <b>CholoBiddut Smart Home</b> and open <b>192.168.4.1</b>.</div></div></div>";
  ...
  s += "<script>";
  s += "let ws;function conn(){ws=new WebSocket('ws://'+location.hostname+':81/');";
  s += "ws.onopen=()=>ws.send('get');";
  ...
  s += "ws.onclose=()=>setTimeout(conn,500)}conn();";
  ...
  s += "function toggleSetup(){let el=document.getElementById('setupInfo');el.classList.toggle('hidden');}";
  s += "</script></body></html>";
  return s;
}
...
void updateStatusLed() {
  if (portalRunning || WiFi.status() != WL_CONNECTED) blinkLedFast();
  else blinkLedSlow();
}
